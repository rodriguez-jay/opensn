  ...
  for (const auto& groupset : groupsets)
  {
        ...
        if (isSurf)
        {
          ...
          for (unsigned int fi = 0; fi < num_face_nodes; ++fi)
          {
            ...
            if (gset == 0)
            {
              ...
              // std::cout << "Cell " << cell_id << " Face : " << f << std::endl;
              // std::cout << "Points : "
              //         << node_vec[0] << " " 
              //         << node_vec[1] << " "
              //         << node_vec[2] << " " 
    ...
    for (const auto& surf_id : surf_tags)
    {
      ...
      for (const auto&vec : data_vectors)
      {
        ...
        // std::cout << "Written Mu : " << vec[3] << std::endl;
        mu.push_back(vec[3]);
...
DiscreteOrdinatesProblemIO::ReadSurfaceAngularFluxes(
  DiscreteOrdinatesProblem& do_problem,
  const std::string& file_base,
  std::vector<std::string>& surfaces)
{
  ...
  const auto& grid = do_problem.GetGrid();
  const auto& supported_bd_ids = grid->GetBoundaryIDMap();
  const auto& supported_bd_names = grid->GetBoundaryNameMap();
  const auto& discretization = do_problem.GetSpatialDiscretization();
  const auto& groupsets = do_problem.GetGroupsets();
  auto num_groupsets = groupsets.size();
  ...
  for (const auto& groupset : groupsets)
  {
    ...
    for (const auto& surface : surfaces)
    {
      ...
      for (size_t ci = 0; ci < num_cells; ++ci)
      {
        ...
        {
          centroid[0] += surf_map.nodes_x[node_stride];
          centroid[1] += surf_map.nodes_y[node_stride];
          centroid[2] += surf_map.nodes_z[node_stride];
          uint64_t qx = std::round(surf_map.nodes_x[node_stride] / coordinate_epsilon);
          uint64_t qy = std::round(surf_map.nodes_y[node_stride] / coordinate_epsilon);
          uint64_t qz = std::round(surf_map.nodes_z[node_stride] / coordinate_epsilon);
          QuantizedCoordinate vertex = {qx, qy, qz};
        ...
        const double reciprocal = 1.0 / num_face_nodes;
        for (double& coord : centroid)
          coord *= reciprocal;
        uint64_t qx = std::round(centroid[0] / coordinate_epsilon);
        uint64_t qy = std::round(centroid[1] / coordinate_epsilon);
        uint64_t qz = std::round(centroid[2] / coordinate_epsilon);
        QuantizedCoordinate key = {qx, qy, qz};
        cell_map[key] = ci;
      }
      // std::cout << "Num Faces : " << Faces.size() << std::endl;
      // exit(0);
      // std::cout << "Stride : " << stride << std::endl;
      // std::cout << "Psi : " << surf_data.psi.size() << std::endl;

      surf_map.cell_map = cell_map;
      surf_map.cell_stride = cell_stride;
      surf_map.faces = Faces;
